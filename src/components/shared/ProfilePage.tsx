...
export function ProfilePage() {
  ...
  const getBmiCategory = (bmiVal: number) => {
    if (bmiVal < 18.5) return { label: "Kam vazn", color: "text-amber-600" };
    if (bmiVal < 25) return { label: "Normal", color: "text-emerald-600" };
    if (bmiVal < 30)
      return { label: "Ortiqcha vazn", color: "text-orange-600" };
        ...
        <CardContent>
          {userKlublar.length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              ...
              <Link href="/sportsman/klublar" className="mt-4">
                Klublarni ko'rish
              </Link>
            </div>
          ) : (
